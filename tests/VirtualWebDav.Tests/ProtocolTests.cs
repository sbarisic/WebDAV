...
public sealed class ProtocolTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        client = new HttpClient { BaseAddress = server.ListeningUri };
    }
    public async Task DisposeAsync() { client.Dispose(); await server.DisposeAsync(); }
    ...
    private async Task Put(string path, string value) { using var r = await Send("PUT", path, value); Assert.True(r.IsSuccessStatusCode); }
    ...
    public async Task FullDriveLifecycleAndUnicode()
    {
        ...
        var listing = await Send("PROPFIND", "/Folder", null, ("Depth", "1")); Status(207, listing);
    ...
    public async Task RangesAndHttpPreconditions()
    {
        ...
        var get = await Send("GET", "/a"); var tag = get.Headers.ETag!.ToString();
        var range = await Send("GET", "/a", null, ("Range", "bytes=2-5")); Status(206, range);
    ...
    {
        var locked = await Send("LOCK", "/new", LockBody, ("Depth", "0")); Status(201, locked);
        var token = locked.Headers.GetValues("Lock-Token").Single();
    ...
    {
        Status(201, await Send("MKCOL", "/dir")); await Put("/dir/a", "a"); await Put("/source", "b");
    ...
    {
        Status(201, await Send("MKCOL", "/dir")); await Put("/dir/a", "a");
    ...
    {
        Status(201, await Send("MKCOL", "/Folder")); await Put("/Folder/file", "data");
    ...
    {
        await Put("/a", "old");
        var tag = (await Send("HEAD", "/a")).Headers.ETag!.ToString();
    ...
    public async Task TruncatedNetworkUploadDoesNotCommit()
    {
        ...
        using (var tcp = new TcpClient())
        {
            ...
            var data = Encoding.ASCII.GetBytes($"PUT /a HTTP/1.1\r\nHost: localhost:{client.BaseAddress.Port}\r\nContent-Length: 100000\r\nConnection: close\r\n\r\npartial");
    ...
    [Theory]
    [InlineData("/../x")][InlineData("/a//b")][InlineData("/a//")][InlineData("/CON.txt")][InlineData("/a.")][InlineData("/a\\b")]
    ...
    {
        await Put("/a", "original");
        await using (var write = await fs.BeginWriteAsync("/a", default)) await write.Stream.WriteAsync(Encoding.UTF8.GetBytes("uncommitted"));
        Assert.Equal("original", await client.GetStringAsync("/a"));
