...
public sealed class StreamingTests
{
    ...
    public async Task LargeTransfersUseBoundedBuffersAndDisposeStreams()
    {
        ...
        var buffer = new byte[32768]; long received = 0;
        int n;
        while ((n = await data.ReadAsync(buffer)) > 0) { received += n; Assert.Equal(-1, buffer.AsSpan(0, n).IndexOfAnyExcept((byte)0)); }
        Assert.Equal(length, received);
        // Server disposal waits until response processing has finished.
        for (int i = 0; i < 100 && !provider.ReadDisposed; i++) await Task.Delay(10);
    ...
    private sealed class ZeroStream(long length, Action<int>? onRead = null, Action? onDispose = null) : Stream
    {
        ...
        public override int Read(Span<byte> buffer)
        {
            ...
            int n = (int)Math.Min(buffer.Length, remaining); buffer[..n].Clear(); remaining -= n; return n;
        }
        ...
        protected override void Dispose(bool disposing) { onDispose?.Invoke(); base.Dispose(disposing); }
    }
    ...
    private sealed class SinkStream(Action<int> onWrite) : Stream
    {
        ...
        public override ValueTask WriteAsync(ReadOnlyMemory<byte> buffer, CancellationToken ct = default) { ct.ThrowIfCancellationRequested(); onWrite(buffer.Length); return ValueTask.CompletedTask; }
    }
    ...
    private sealed class StreamingProvider(long length) : IVirtualFileSystem
    {
        ...
        private sealed class Session : IVirtualWriteSession
        {
            ...
            public Session(StreamingProvider owner) { this.owner = owner; Stream = new SinkStream(n => { owner.Written += n; owner.LargestWrite = Math.Max(n, owner.LargestWrite); }); }
            public Stream Stream { get; }
            public Task CommitAsync(CancellationToken ct) { owner.Committed = true; return Task.CompletedTask; }
            public async ValueTask DisposeAsync() { owner.WriteDisposed = true; await Stream.DisposeAsync(); }
        }
