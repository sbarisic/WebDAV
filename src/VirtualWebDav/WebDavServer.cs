...
public sealed partial class WebDavServer : IAsyncDisposable
{
    ...
    private WebDavServer(IVirtualFileSystem provider, WebDavServerOptions options)
    {
        fs = provider;
    ...
    public static async Task<WebDavServer> StartAsync(IVirtualFileSystem provider, WebDavServerOptions? options = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await server.app.StartAsync(cancellationToken);
        }
        catch
        {
    ...
    private async Task HandleAsync(HttpContext context)
    {
        ...
        try
        {
            ...
            string raw = context.Features.Get<IHttpRequestFeature>()?.RawTarget ?? context.Request.Path.Value ?? "/";
            string path = DecodePath(raw.Split('?')[0]);
            ...
            switch (context.Request.Method)
            {
                case "OPTIONS":
                    ...
                    break;
                case "PROPFIND":
                    await PropFindAsync(context, path);
                    break;
                case "GET":
                case "HEAD":
                    await ReadAsync(context, path);
                    break;
                case "LOCK":
                    await LockAsync(context, path);
                    break;
                case "UNLOCK":
                    await UnlockAsync(context, path);
                    break;
                default:
                    await MutateAsync(context, path);
                    break;
            }
        }
        ...
        catch (Exception e)
        {
            ...
            int status = e switch
            {
                ...
                {
                    FileSystemError.NotFound => 404,
                    FileSystemError.Forbidden => 403,
                    FileSystemError.AlreadyExists => 412,
                    FileSystemError.Conflict => 409,
                    FileSystemError.InsufficientStorage => 507,
                    FileSystemError.NotSupported => 405,
                    _ => 500
                },
            ...
            if (status == 500)
            {
                app.Logger.LogError(e, "WebDAV provider/request failure");
            }
    ...
    private void ValidateRequest(HttpContext c)
    {
        ...
        if (host.Port != options.Port || !(host.Host.Equals("localhost", StringComparison.OrdinalIgnoreCase) || IPAddress.TryParse(host.Host.Trim('[', ']'), out var ip) && IPAddress.IsLoopback(ip)))
        {
            throw new DavException(403);
        }

        if (c.Connection.RemoteIpAddress is not { } remote || !IPAddress.IsLoopback(remote))
        {
            throw new DavException(403);
        }

        if (c.Request.Headers.ContainsKey("Origin") || c.Request.Headers.ContainsKey("Sec-Fetch-Site"))
        {
            throw new DavException(403);
        }
    }
    ...
    private static string DecodePath(string escaped)
    {
        ...
        if (escaped.Contains("%2f", StringComparison.OrdinalIgnoreCase) || escaped.Contains("%5c", StringComparison.OrdinalIgnoreCase))
        {
            throw new DavException(400);
        }
    ...
    {
        if (c.Request.ContentLength > options.MaxXmlBytes)
        {
            throw new DavException(413);
        }
        ...
        {
            if (buffer.Length + read > options.MaxXmlBytes)
            {
                throw new DavException(413);
            }
        ...
        buffer.Position = 0;
        using var reader = XmlReader.Create(buffer, new XmlReaderSettings { Async = true, DtdProcessing = DtdProcessing.Prohibit, XmlResolver = null, MaxCharactersInDocument = options.MaxXmlBytes });
    ...
    private async Task<VirtualEntry> RequiredAsync(string path, CancellationToken ct) => await fs.GetEntryAsync(path, ct) ?? throw new DavException(404);
    private async Task ReadAsync(HttpContext c, string path)
    {
        ...
        if (!await PreconditionsAsync(c, path, entry, true))
        {
            return;
        }
        ...
        if (stream.CanSeek)
        {
            ...
            string range = c.Request.Headers.Range.ToString();
            string ifRange = c.Request.Headers.IfRange.ToString();
            bool allowRange = ifRange.Length == 0 || ifRange == entry.ETag || DateTimeOffset.TryParse(ifRange, out var date) && entry.Modified.ToUnixTimeSeconds() <= date.ToUnixTimeSeconds();
            if (allowRange && range.StartsWith("bytes=") && !range.Contains(','))
            {
                string[] parts = range[6..].Split('-');
                if (parts.Length != 2)
                {
                    throw new DavException(400);
                }
                ...
                if (parts[0].Length == 0)
                {
                    ...
                    {
                        throw new DavException(416);
                    }

                    start = Math.Max(0, count - suffix);
                    end = count - 1;
                }
                else
                {
                    ...
                    end = parts[1].Length == 0 ? count - 1 : long.TryParse(parts[1], out long parsed) ? Math.Min(parsed, count - 1) : throw new DavException(400);
                }

                if (start < 0 || start >= count || end < start)
                {
                    c.Response.StatusCode = 416;
                    c.Response.ContentLength = 0;
                    c.Response.Headers.ContentRange = $"bytes */{count}";
                    return;
                }
                ...
                count = end - start + 1;
                c.Response.ContentLength = count;
            }
        }
        ...
        while (count > 0)
        {
            int n = await stream.ReadAsync(buffer.AsMemory(0, (int)Math.Min(buffer.Length, count)), c.RequestAborted);
            if (n == 0)
            {
                throw new IOException("Provider stream shorter than advertised length.");
            }
    ...
    {
        if (c.Request.Method is not ("PUT" or "MKCOL" or "DELETE" or "COPY" or "MOVE" or "PROPPATCH"))
        {
            throw new DavException(405);
        }
        ...
        if (path == "/" && c.Request.Method != "PROPPATCH")
        {
            throw new DavException(403);
        }
        ...
        {
            CheckLocks(c, path, c.Request.Method is "DELETE" or "MOVE");
        }
        ...
        switch (c.Request.Method)
        {
            case "PUT":
                ...
                if (c.Request.Headers.ContainsKey("Content-Range"))
                {
                    throw new DavException(400);
                }
            ...
            case "MKCOL":
                ...
                if (await c.Request.Body.ReadAsync(new byte[1], ct) != 0)
                {
                    throw new DavException(415);
                }

                await fs.CreateDirectoryAsync(path, ct);
                c.Response.StatusCode = 201;
                break;
            case "DELETE":
                await RequiredAsync(path, ct);
                await fs.DeleteAsync(path, ct);
            ...
            case "MOVE":
                await RequiredAsync(path, ct);
                string destination = Destination(c);
                bool caseRename = c.Request.Method == "MOVE" && destination.Equals(path, StringComparison.OrdinalIgnoreCase) && destination != path;
                if (destination == "/" || !caseRename && (VirtualPath.Within(destination, path) || VirtualPath.Within(path, destination)))
                {
                    throw new DavException(403);
                }

                var existing = await fs.GetEntryAsync(destination, ct);
                string overwrite = c.Request.Headers["Overwrite"].ToString();
                if (overwrite is not ("" or "T" or "F"))
                {
                    throw new DavException(400);
                }

                if (existing != null && overwrite == "F")
                {
                ...
                string depth = c.Request.Headers["Depth"].ToString();
                if (depth is not ("" or "0" or "infinity") || c.Request.Method == "MOVE" && depth == "0")
                {
                    throw new DavException(400);
                }
                ...
                if (c.Request.Method == "COPY")
                {
                    await fs.CopyAsync(path, destination, overwrite != "F", depth != "0", ct);
                }
                else
                {
                    await fs.MoveAsync(path, destination, overwrite != "F", ct);
                }

                if (c.Request.Method == "MOVE" && !caseRename)
                {
                    locks.RemoveAll(l => VirtualPath.Within(l.Path, path));
                }

                c.Response.StatusCode = existing == null ? 201 : 204;
                break;
            case "PROPPATCH":
                await PropPatchAsync(c, path);
                break;
        }
    }
    ...
    {
        string value = c.Request.Headers["Destination"].ToString();
    ...
    {
        if (!Uri.TryCreate(value, UriKind.Absolute, out var uri) || uri.Scheme != "http" || uri.Port != options.Port || !uri.Host.Equals(c.Request.Host.Host.Trim('[', ']'), StringComparison.OrdinalIgnoreCase) || uri.Query.Length > 0 || uri.Fragment.Length > 0 || uri.UserInfo.Length > 0)
        {
            throw new DavException(errorStatus);
        }

        // Keep the original escaped path: System.Uri normalizes dot segments away.
        int slash = value.IndexOf('/', value.IndexOf("://", StringComparison.Ordinal) + 3);
