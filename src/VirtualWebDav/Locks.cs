...
public sealed partial class WebDavServer
{
    ...
    {
        string text = c.Request.Headers["If"].ToString();
        ...
        while (i < text.Length)
        {
            ...
            var terms = new List<IfTerm>();
            White();
            while (i < text.Length && text[i] != ')')
            {
                bool negated = false;
                if (text.AsSpan(i).StartsWith("Not ", StringComparison.Ordinal))
                {
                    negated = true;
                    i += 4;
                    White();
                ...
                bool etag = text[i] == '[';
                string value = etag ? Between('[', ']') : Between('<', '>');
                terms.Add(new(negated, value, etag));
                White();
            }

            if (terms.Count == 0 || i >= text.Length || text[i++] != ')')
            {
                throw new DavException(400);
            }

            lists.Add(new(current, terms));
            White();
        }
    ...
    {
        string match = c.Request.Headers.IfMatch.ToString();
        bool Matches(string values, bool weak) => values.Split(',').Select(v => v.Trim()).Any(v => v == "*" ? entry != null : entry != null && (weak ? v.Replace("W/", "") == entry.ETag.Replace("W/", "") : !v.StartsWith("W/") && v == entry.ETag));
        if (match.Length > 0 && !Matches(match, false))
        {
            throw new DavException(412);
        }

        if (match.Length == 0 && entry != null && DateTimeOffset.TryParse(c.Request.Headers.IfUnmodifiedSince, out var unmodified) && entry.Modified.ToUnixTimeSeconds() > unmodified.ToUnixTimeSeconds())
        {
            throw new DavException(412);
        }

        string none = c.Request.Headers.IfNoneMatch.ToString();
        if (none.Length > 0 && Matches(none, true))
        {
            ...
            c.Response.ContentLength = null;
            return false;
        }

        if (read && none.Length == 0 && entry != null && DateTimeOffset.TryParse(c.Request.Headers.IfModifiedSince, out var modified) && entry.Modified.ToUnixTimeSeconds() <= modified.ToUnixTimeSeconds())
        {
            c.Response.StatusCode = 304;
            c.Response.ContentLength = null;
            return false;
        ...
        foreach (var group in lists.GroupBy(l => l.Path, StringComparer.OrdinalIgnoreCase))
        {
            ...
            foreach (var list in group)
            {
                ...
                {
                    any = true;
                    foreach (var term in list.Terms.Where(t => !t.Negated && !t.ETag))
                    {
                        submitted.Add(term.Value);
                    }
                }
            }
    ...
    {
        foreach (string? part in c.Request.Headers["Timeout"].ToString().Split(',').Select(p => p.Trim()))
        {
            if (part == "Infinite")
            {
            ...
            if (part.StartsWith("Second-") && long.TryParse(part[7..], out long seconds) && seconds > 0)
            {
    ...
    private async Task LockAsync(HttpContext c, string path)
    {
        ...
        if (xml == null)
        {
            ...
            locks.Remove(old);
            locks.Add(old with { Expires = DateTimeOffset.UtcNow + Timeout(c) });
            await XmlAsync(c, new XElement(Dav + "prop", LockDiscovery(path)), 200);
            return;
        }

        if (xml.Name != Dav + "lockinfo" || xml.Element(Dav + "lockscope")?.Element(Dav + "exclusive") == null || xml.Element(Dav + "locktype")?.Element(Dav + "write") == null)
        {
            throw new DavException(400);
        }

        string depth = c.Request.Headers["Depth"].ToString();
        if (depth is not ("" or "0" or "infinity"))
        {
            throw new DavException(400);
        }

        bool recursive = depth != "0";
        if (locks.Any(l => Covers(l, path) || recursive && VirtualPath.Within(l.Path, path)))
        {
            throw new DavException(423, "no-conflicting-lock");
        }
    ...
    private async Task UnlockAsync(HttpContext c, string path)
    {
        ...
        string token = c.Request.Headers["Lock-Token"].ToString();
        ...
        if (item == null)
        {
            throw new DavException(409, "lock-token-matches-request-uri");
        }
    ...
    private XElement LockDiscovery(string path) => new(Dav + "lockdiscovery", locks.Where(l => Covers(l, path)).Select(l => new XElement(Dav + "activelock", new XElement(Dav + "locktype", new XElement(Dav + "write")), new XElement(Dav + "lockscope", new XElement(Dav + "exclusive")), new XElement(Dav + "depth", l.InfiniteDepth ? "infinity" : "0"), l.Owner == null ? null : new XElement(l.Owner), new XElement(Dav + "timeout", "Second-" + Math.Max(1, (int)(l.Expires - DateTimeOffset.UtcNow).TotalSeconds)), new XElement(Dav + "locktoken", new XElement(Dav + "href", l.Token)), new XElement(Dav + "lockroot", new XElement(Dav + "href", VirtualPath.Href(l.Path, false))))));
}
