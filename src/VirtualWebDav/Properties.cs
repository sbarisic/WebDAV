...
public sealed partial class WebDavServer
{
    private async Task PropFindAsync(HttpContext c, string path)
    {
        ...
        if (depth is "" or "infinity")
        {
            throw new DavException(403, "propfind-finite-depth");
        }
        ...
        if (depth == "1" && entry.IsDirectory)
        {
            ...
            {
                if (child.Name.Contains('/') || !names.Add(child.Name))
                {
                    throw new InvalidOperationException("Provider returned ambiguous names.");
                }

                string childPath = VirtualPath.Normalize((path == "/" ? "" : path) + "/" + child.Name);
    ...
    private async Task<XElement> PropertyResponseAsync(string path, VirtualEntry entry, XElement? selector, XElement? include, CancellationToken ct)
    {
        var properties = new List<XElement>
        {
            ...
            new(Dav + "supportedlock", new XElement(Dav + "lockentry", new XElement(Dav + "lockscope", new XElement(Dav + "exclusive")), new XElement(Dav + "locktype", new XElement(Dav + "write")))),
            LockDiscovery(path)
        };
        ...
        else if (selector?.Name == Dav + "prop")
        {
            ...
            if (found.Length > 0)
            {
                response.Add(PropStat(found, 200));
            }

            if (missing.Length > 0)
            {
                response.Add(PropStat(missing, 404));
            }

            if (requested.Length == 0)
            {
                response.Add(PropStat([], 200));
            }
        }
        else
        {
    ...
    private static XElement PropStat(IEnumerable<XElement> properties, int status) => new(Dav + "propstat", new XElement(Dav + "prop", properties), new XElement(Dav + "status", $"HTTP/1.1 {status} {Microsoft.AspNetCore.WebUtilities.ReasonPhrases.GetReasonPhrase(status)}"));
    private async Task PropPatchAsync(HttpContext c, string path)
    {
        var entry = await RequiredAsync(path, c.RequestAborted);
        var request = await ReadXmlAsync(c);
        if (request?.Name != Dav + "propertyupdate" || !request.HasElements)
        {
            throw new DavException(400);
        }
        ...
        {
            if (instruction.Name != Dav + "set" && instruction.Name != Dav + "remove")
            {
                throw new DavException(400);
            }
            ...
            {
                changes[p.Name] = instruction.Name == Dav + "set" ? new XElement(p) : null;
            }
        }
