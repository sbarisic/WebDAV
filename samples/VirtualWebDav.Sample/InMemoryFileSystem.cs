...
public sealed class InMemoryFileSystem : CustomFileSystem
{
    private sealed class Node(bool directory)
    {
        ...
        public Node Clone() => new(Directory)
        {
            Data = Data.ToArray(),
            Properties = Properties.ToDictionary(p => p.Key, p => new XElement(p.Value))
        };
        public void Touch()
        {
    ...
    {
        if (!nodes.TryGetValue(VirtualPath.Parent(path), out var parent) || !parent.Directory)
        {
            throw new VirtualFileSystemException(FileSystemError.Conflict);
        }
    }

    private static string Path(string path, CancellationToken ct)
    {
    ...
    {
        if (nodes.TryGetValue(VirtualPath.Parent(path), out var parent))
        {
            parent.Touch();
        }
    }

    public override Task<VirtualEntry?> GetEntryAsync(string path, CancellationToken cancellationToken)
    {
    ...
    public override async IAsyncEnumerable<VirtualEntry> EnumerateAsync(string path, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        path = Path(path, cancellationToken);
        VirtualEntry[] entries;
        lock (gate)
        {
            if (!Required(path).Directory)
            {
                throw new VirtualFileSystemException(FileSystemError.Conflict);
            }
    ...
    public override Task<Stream> OpenReadAsync(string path, CancellationToken cancellationToken)
    {
        path = Path(path, cancellationToken);
        lock (gate)
        {
    ...
    public override Task<IVirtualWriteSession> BeginWriteAsync(string path, CancellationToken cancellationToken)
    {
        path = Path(path, cancellationToken);
        lock (gate)
        {
            ParentExists(path);
            if (nodes.TryGetValue(path, out var node) && node.Directory)
            {
                throw new VirtualFileSystemException(FileSystemError.Conflict);
            }
        }
    ...
    private sealed class WriteSession(InMemoryFileSystem owner, string path) : IVirtualWriteSession
    {
        ...
        public Task CommitAsync(CancellationToken cancellationToken)
        {
            ...
            lock (owner.gate)
            {
                ...
                if (owner.nodes.TryGetValue(path, out var current) && current.Directory)
                {
                    throw new VirtualFileSystemException(FileSystemError.Conflict);
                }
                ...
                node.Data = data;
                node.Touch();
                owner.nodes[path] = node;
                owner.TouchParent(path);
                completed = true;
            }
    ...
    public override Task CreateDirectoryAsync(string path, CancellationToken cancellationToken)
    {
        ...
        lock (gate)
        {
            ...
            {
                throw new VirtualFileSystemException(FileSystemError.AlreadyExists);
            }

            nodes.Add(path, new(true));
            TouchParent(path);
        }
    ...
    public override Task DeleteAsync(string path, CancellationToken cancellationToken)
    {
        ...
        lock (gate)
        {
            ...
            Required(path);
            foreach (string? key in nodes.Keys.Where(p => VirtualPath.Within(p, path)).ToArray())
            {
                nodes.Remove(key);
            }
    ...
    public override Task CopyAsync(string source, string destination, bool overwrite, bool recursive, CancellationToken cancellationToken) => Transfer(source, destination, overwrite, recursive, false, cancellationToken);
    public override Task MoveAsync(string source, string destination, bool overwrite, CancellationToken cancellationToken) => Transfer(source, destination, overwrite, true, true, cancellationToken);
    private Task Transfer(string source, string destination, bool overwrite, bool recursive, bool move, CancellationToken ct)
    {
        source = Path(source, ct);
        destination = Path(destination, ct);
        lock (gate)
        {
            Required(source);
            ParentExists(destination);
            bool caseRename = move && destination.Equals(source, StringComparison.OrdinalIgnoreCase) && destination != source;
            if (!caseRename && (VirtualPath.Within(destination, source) || VirtualPath.Within(source, destination)))
            {
                throw new VirtualFileSystemException(FileSystemError.Forbidden);
            }

            if (nodes.ContainsKey(destination) && !overwrite)
            {
                throw new VirtualFileSystemException(FileSystemError.AlreadyExists);
            }
            ...
            foreach (string? key in nodes.Keys.Where(p => VirtualPath.Within(p, destination)).ToArray())
            {
                nodes.Remove(key);
            }
    ...
    public override Task<IReadOnlyList<XElement>> GetPropertiesAsync(string path, CancellationToken cancellationToken)
    {
        path = Path(path, cancellationToken);
        lock (gate)
        {
            return Task.FromResult<IReadOnlyList<XElement>>(Required(path).Properties.Values.Select(p => new XElement(p)).ToArray());
        }
    }

    public override Task PatchPropertiesAsync(string path, IReadOnlyList<XElement> set, IReadOnlyList<XName> remove, CancellationToken cancellationToken)
    {
        path = Path(path, cancellationToken);
        lock (gate)
        {
            ...
            {
                updated[property.Name] = new XElement(property);
            }
